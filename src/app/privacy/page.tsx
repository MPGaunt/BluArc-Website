    ...
    <PageShell company={content.company}>
      <section className="bg-[#eef6ff] px-5 py-12">
        ...
        <div className="rounded-[2rem] bg-white/90 p-6 shadow-2xl shadow-slate-300/60 sm:p-8">
          <p className="text-sm font-black uppercase tracking-wide text-sky-700">Privacy</p>
          <h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Privacy links that are ready to paste.</h1>
          <p className="mt-4 max-w-2xl leading-7 text-slate-600">
            Each app has a dedicated policy URL for App Store Connect, kept simple and easy to update.
          </p>
        </div>
        <div className="mt-6 divide-y divide-slate-200 overflow-hidden rounded-[2rem] border border-white bg-white/95 shadow-xl shadow-slate-300/50">
          {content.apps.map((app) => (
            <Link key={app.id} href={`/privacy/${app.slug}`} className="flex items-center justify-between gap-4 p-5 transition hover:bg-sky-50">
              <span>
                <span className="block font-black text-slate-950">{app.name}</span>
                <span className="mt-1 block text-sm text-slate-600">/privacy/{app.slug}</span>
              </span>
